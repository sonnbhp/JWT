...
1.  **`gnoc_lib/`**: Thư viện chứa logic đăng nhập (JWT), gọi API, và gửi tin nhắn NetChat. (Đã tối ưu hóa cho EXE).
2.  **`build_exe.ps1`**: Kịch bản đóng gói thành 1 file EXE duy nhất cực kỳ ổn định.
3.  **`AGENTS.md` & `RULES.md`**: Bộ quy tắc giúp AI (Claude) hoạt động đúng vai trò Manager, Core, và UX.
4.  **`.claude/knowledge/`**: Nơi lưu trữ các bài học kinh nghiệm (KI) để AI không mắc lại lỗi cũ.
